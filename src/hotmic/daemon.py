...
class VoiceDaemon:
    ...
    def _on_release(self, key: keyboard.Key) -> None:
        """Handle key release. Only first released key triggers stop."""
        key = self._normalize_key(key)
        if key in self.required_keys:
            was_all_pressed = self.pressed_keys == self.required_keys
            self.pressed_keys.discard(key)
            # Only trigger stop on the first key release (transition from all-pressed)
            if was_all_pressed and self.is_recording:
    ...
    def _start_recording(self) -> None:
        """Start recording audio. Called once, guarded by lock in _on_press."""
        self._stopping = False
        self._transcription_done = False
        self._last_feed_index = 0
        ...
        try:
            ...
            # PortAudio corrupted. Write sentinel file so menubar supervisor
            # can detect and do a clean restart.
            self._log("Mic failed, signaling menubar to restart...")
            try:
                (DEFAULT_CONFIG_DIR / "mic_error.flag").write_text("1")
            except Exception:
                pass
            self.is_recording = False
            if self.overlay:
                self.overlay.show("请点击菜单栏 一键修复", status="麦克风异常")
                threading.Timer(3.0, self.overlay.hide).start()
            return

        # Sound + overlay AFTER recording is live
    ...
    def _get_volume_indicator(self) -> str:
        """Get a volume bar from recent audio for overlay display."""
        if not self.recorder.audio_data:
            return ""
        try:
            import numpy as np
            # Use last chunk of audio data
            recent = self.recorder.audio_data[-1] if self.recorder.audio_data else None
            if recent is None:
                return ""
            rms = float(np.sqrt(np.mean(recent ** 2)))
            # Map RMS to 0-5 bars (typical speech RMS: 0.01-0.15)
            level = min(5, int(rms * 40))
            bars = "█" * level + "░" * (5 - level)
            return f"🎙 {bars}"
        except Exception:
            return "🎙"
    ...
    def _start_chunk_timer(self) -> None:
        ...
        First 5s: only accumulate audio + show volume animation (no inference).
        After 5s: start feeding audio to ASR with VAD-based chunking.
        """
        self._chunk_count = 0
        self._recording_start_time = time.time()
        interval = self.config.feed_interval if self.config.asr_backend == "qwen3" else 2.0
        warmup_sec = 5.0  # accumulate this much audio before first inference
        ...
        # Volume indicator timer (updates overlay every 0.2s before text appears)
        def update_volume():
            if not self.is_recording:
                return
            if self.overlay and not self._has_text:
                vol = self._get_volume_indicator()
                elapsed = time.time() - self._recording_start_time
                remaining = max(0, warmup_sec - elapsed)
                if remaining > 0:
                    status = f"Listening... {remaining:.0f}s"
                else:
                    status = "Recording..."
                if vol:
                    self.overlay.show(vol, status=status)
            if self.is_recording:
                self._vol_timer = threading.Timer(0.2, update_volume)
        ...
        # Start feed timer from 0.5s (but do_chunk skips inference until warmup done)
        with self._chunk_lock:
            self._chunk_timer = threading.Timer(interval, do_chunk)
            self._chunk_timer.daemon = True
            self._chunk_timer.start()
    ...
    def _stop_chunk_timer(self) -> None:
        """Stop the chunk timer and volume timer."""
        with self._chunk_lock:
    ...
    def _transcribe_current_chunk(self) -> None:
        ...
        try:
            ...
            if text and self.overlay:
                self._has_text = True  # stop volume indicator, show text
    ...
    def _stop_recording(self) -> None:
        """Stop recording and process audio. Guarded against double-call."""
        if self._stopping or not self.is_recording:
            return
        self._stopping = True
        self.is_recording = False
    ...
    def _process_audio(self, audio) -> None:
        ...
        # Guard: if another thread already produced a result, skip
        if getattr(self, '_transcription_done', False):
            return
        if audio is None:
        ...
        try:
            ...
            if result.success:
                self._transcription_done = True  # block any duplicate calls
                self._log(f"Transcribed ({result.duration_seconds:.1f}s): {result.text[:50]}...")

                # Post-processing: remove fillers + merge short sentences
                text = result.text
                if self.config.remove_fillers:
                    from .text_cleaner import remove_fillers
                    text = remove_fillers(text, self.config.language)
                    if text != result.text:
                        self._log(f"Cleaned: {text[:50]}...")
                        result = TranscriptionResult(
                            text=text, duration_seconds=result.duration_seconds,
                ...
                # Don't show error in overlay — just hide silently
                # (avoids flicker from duplicate stop calls)
                if self.overlay and not self._transcription_done:
                    self.overlay.hide()
    ...
    def _self_restart(self) -> None:
        """Restart the daemon process to recover from PortAudio corruption."""
        import time as _time
        _time.sleep(0.5)
        self._log("Self-restarting daemon...")
        ...
        try:
            subprocess.Popen(
                [sys.executable, str(exec_path), "daemon", "start", "--background", "--quiet"],
                start_new_session=True,
            )
            self._log("Self-restart initiated")
        except Exception as e:
            self._log(f"Self-restart failed: {e}")
